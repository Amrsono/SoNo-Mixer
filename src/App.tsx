...
function App() {
    ...
    return (
        <div className={`min-h-screen sm:min-h-screen ${backgroundTheme === 'light' ? 'bg-gradient-to-br from-gray-100 via-gray-50 to-gray-200' : 'bg-gradient-to-br from-purple-900 via-pink-800 to-blue-900'} ${getTextColorClass()} p-4 sm:p-8 font-sans selection:bg-red-500/30 relative overflow-hidden flex flex-col`}>
            {/* Background Layers */}
            {renderBackground()}
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-40 p-4">
                    <div className={`${
                        ...
                        <button
                            ...
                            className={`w-full mt-6 p-3 rounded-xl font-bold transition-all ${
                                backgroundTheme === 'light'
            ...
            <header className="mb-4 sm:mb-12 flex justify-between items-start sm:items-center group flex-shrink-0">
                ...
                <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => setShowSettings(!showSettings)} className={`p-2 sm:p-2 rounded-full transition-colors ${
                        backgroundTheme === 'light' 
            ...
            <main className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8 flex-1 overflow-y-auto sm:overflow-visible">
                <div className="lg:col-span-2 space-y-4 sm:space-y-8 overflow-y-auto sm:overflow-visible">
                    {/* Vocal Booth */}
                    <section className={`rounded-2xl sm:rounded-3xl p-4 sm:p-8 backdrop-blur-xl relative overflow-hidden group flex-shrink-0 border transition-all ${
                        backgroundTheme === 'light'
                        ...
                        <div className={`h-24 sm:h-32 rounded-xl sm:rounded-2xl flex items-center justify-center relative overflow-hidden p-2 sm:p-4 border transition-all ${
                            backgroundTheme === 'light'
                        ...
                        <div className="mt-4 sm:mt-8 flex items-center justify-between gap-2">
                            <div className="flex gap-2 sm:gap-4 flex-wrap">
                                {isRecording ? (
                                    <button onClick={stopRecording} className={`flex items-center gap-1 sm:gap-2 px-4 sm:px-8 py-2 sm:py-3 rounded-full font-black text-sm sm:text-base transition-all ${
                                        backgroundTheme === 'light'
                                ...
                                <button onClick={handlePreview} disabled={!recordedBlobUrl} className={`flex items-center gap-1 sm:gap-2 px-4 sm:px-8 py-2 sm:py-3 rounded-full font-black text-sm sm:text-base disabled:opacity-30 transition-all ${
                                    backgroundTheme === 'light'
                                        ? 'bg-gray-400 text-white hover:bg-gray-500'
                                        : 'bg-zinc-800 text-white hover:bg-zinc-700'
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_auto_auto_1fr] gap-2 sm:gap-4 items-center flex-shrink-0 transition-all">
                        {/* Deck A */}
                        <div className={`p-3 sm:p-6 rounded-2xl sm:rounded-3xl border transition-all ${selectedDeck === 'A' ? backgroundTheme === 'light' ? 'bg-orange-100 border-orange-400' : 'bg-orange-500/10 border-orange-500' : backgroundTheme === 'light' ? 'bg-white/70 border-gray-300' : 'bg-zinc-900/50 border-zinc-800'}`} onClick={() => setSelectedDeck('A')}>
                            ...
                            <div className={`h-16 sm:h-24 rounded-lg sm:rounded-xl mb-2 sm:mb-4 flex flex-col items-center justify-center border relative overflow-hidden ${
                                backgroundTheme === 'light'
                                    ? 'bg-gray-300 border-gray-400'
                                    : 'bg-zinc-950 border-zinc-800/50'
                            }`}>
                                <span className={`text-[8px] sm:text-[10px] uppercase font-bold tracking-widest relative z-10 ${
                                    backgroundTheme === 'light' ? 'text-gray-700' : 'text-zinc-600'
                            ...
                            <div className="flex flex-col gap-2 sm:gap-4">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); togglePlay('A'); }} className={`w-full py-2 sm:py-4 rounded-lg sm:rounded-xl font-black flex items-center justify-center gap-1 sm:gap-2 text-xs sm:text-base transition-all ${
                                    isPlayingA
                        ...
                        <div className={`hidden lg:flex flex-col items-center gap-2 p-4 rounded-3xl border transition-all ${
                            backgroundTheme === 'light'
                                ? 'bg-white/70 border-gray-300'
                                : 'bg-zinc-900/30 border-zinc-800/50'
                        }`}>
                            <span className={`text-[10px] font-black ${backgroundTheme === 'light' ? 'text-gray-600' : 'text-zinc-500'}`}>VOICE</span>
                            <div className={`h-40 w-12 rounded-full border flex items-center justify-center relative py-4 ${
                                backgroundTheme === 'light'
                                    ? 'bg-gray-200 border-gray-400'
                        ...
                        <div className="hidden lg:flex flex-col items-center gap-2">
                            ...
                            <div className={`h-48 w-12 rounded-full border flex items-center justify-center relative py-4 ${
                                backgroundTheme === 'light'
                                    ? 'bg-gray-200 border-gray-400'
                                    : 'bg-zinc-900 border-zinc-800'
                            }`}>
                        ...
                        <div className={`p-3 sm:p-6 rounded-2xl sm:rounded-3xl border transition-all ${selectedDeck === 'B' ? backgroundTheme === 'light' ? 'bg-red-100 border-red-400' : 'bg-red-500/10 border-red-500' : backgroundTheme === 'light' ? 'bg-white/70 border-gray-300' : 'bg-zinc-900/50 border-zinc-800'}`} onClick={() => setSelectedDeck('B')}>
                            ...
                            <div className="flex flex-col gap-2 sm:gap-4">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); togglePlay('B'); }} className={`w-full py-2 sm:py-4 rounded-lg sm:rounded-xl font-black flex items-center justify-center gap-1 sm:gap-2 text-xs sm:text-base transition-all ${
                                    isPlayingB
                                        ? backgroundTheme === 'light' ? 'bg-gray-400 text-white' : 'bg-zinc-800 text-white'
                    ...
                    <section className={`rounded-2xl sm:rounded-3xl p-4 sm:p-8 backdrop-blur-xl flex-shrink-0 border transition-all ${
                        backgroundTheme === 'light'
                            ? 'bg-white/80 border-gray-400'
                            : 'bg-zinc-900/50 border-zinc-800'
                    }`}>
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-6 gap-2">
                            ...
                            <button onClick={() => fileInputRef.current?.click()} className={`text-[10px] sm:text-xs font-bold flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 rounded-full transition-all ${
                                backgroundTheme === 'light'
                                    ? 'bg-gray-400 text-white hover:bg-gray-500'
                                    : 'bg-zinc-800 text-white hover:bg-zinc-700'
                        ...
                        <div className="max-h-48 sm:max-h-64 overflow-y-auto pr-2 grid grid-cols-2 gap-2 sm:gap-3 scrollbar-hide">
                            {SAMPLE_BEATS.map((beat) => (
                                <button
                                    ...
                                    className={`p-2 sm:p-4 border rounded-lg sm:rounded-2xl text-left transition-all group ${
                                        backgroundTheme === 'light'
                ...
                <div className="space-y-4 sm:space-y-8 overflow-y-auto sm:overflow-visible">
                    {/* Effects Table */}
                    <section className={`rounded-2xl sm:rounded-3xl p-4 sm:p-8 backdrop-blur-xl flex-shrink-0 border transition-all ${
                        backgroundTheme === 'light'
                            ? 'bg-white/80 border-gray-400'
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:gap-4">
                            {(Object.keys(effects) as Array<keyof typeof effects>).map((fx) => (
                                <button
                                    ...
                                    className={`w-full p-2 sm:p-4 rounded-lg sm:rounded-2xl border flex justify-between items-center transition-all ${
                                        effects[fx]
                    ...
                    {/* Export */}
                    <section className={`rounded-2xl sm:rounded-3xl p-4 sm:p-8 backdrop-blur-xl text-center flex-shrink-0 border transition-all ${
                        backgroundTheme === 'light'
                        ...
                        <button onClick={handleExport} className={`w-full font-black py-3 sm:py-5 rounded-lg sm:rounded-2xl flex items-center justify-center gap-1 sm:gap-2 transition-all text-xs sm:text-base ${
                            backgroundTheme === 'light'
